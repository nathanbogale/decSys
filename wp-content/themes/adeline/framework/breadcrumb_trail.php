 ...
 * @return void
 */
function adeline_breadcrumb_trail($args = array())
{
    // Return if breadcrumbs are disabled
    if (!adeline_has_breadcrumbs() || is_front_page()) {
        return;
    }
    // Yoast breadcrumbs
    if (function_exists('yoast_breadcrumb') && current_theme_supports('yoast-seo-breadcrumbs')) {
        $classes = 'dpr-adeline-breadcrumbs clr';
        if ($breadcrumbs_position = adeline_get_option_value('breadcrumbs_position')) {
    ...
    if (!is_object($breadcrumb)) {
        $breadcrumb = new Adeline_Breadcrumb_Trail($args);
    }
 ...
 * @access public
 */
class Adeline_Breadcrumb_Trail
{
    /**
    ...
    /**
     * Sets up the breadcrumb trail properties.  Calls the `Adeline_Breadcrumb_Trail::add_items()` method
     ...
     *     @type array     $labels         Text labels. @see Adeline_Breadcrumb_Trail::set_labels()
     *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Adeline_Breadcrumb_Trail::set_post_taxonomy()
    ...
    {
        $defaults = array('container' => 'nav', 'before' => '', 'after' => '', 'show_on_front' => false, 'network' => false, 'show_title' => true, 'labels' => array(), 'post_taxonomy' => array(), 'echo' => true);
    ...
    public function get_trail()
    {
        ...
        if (0 < $item_count) {
            ...
            $breadcrumb .= '<ul class="trail-items"' . adeline_get_schema_markup('breadcrumb') . '>';
            ...
            foreach ($this->items as $item) {
                ...
                $item = !empty($matches) ? sprintf('%s<span' . adeline_get_schema_markup('author_name') . '>%s</span>%s', $matches[1], $matches[2], $matches[3]) : sprintf('<span' . adeline_get_schema_markup('author_name') . '>%s</span>', $item);
                ...
                $attributes = 'class="' . $item_class . '"' . adeline_get_schema_markup('breadcrumb_list') . '';
                // Build the meta position HTML.
                $meta = sprintf('<meta content="%s"' . adeline_get_schema_markup('position') . ' />', absint($item_position));
                // Build the list item.
            ...
            // Postion class
            $p_class = '';
            if ('' != adeline_get_option_value('breadcrumbs_position')) {
                $p_class = ' position-' . adeline_get_option_value('breadcrumbs_position');
            }
            // Wrap the breadcrumb trail.
            $breadcrumb = sprintf('<%1$s aria-label="%2$s" class="dpr-adeline-breadcrumbs' . $p_class . '"' . adeline_get_schema_markup('breadcrumb_itemprop') . '>%3$s%4$s%5$s</%1$s>', tag_escape($this->args['container']), esc_attr($this->labels['aria_label']), $this->args['before'], $breadcrumb, $this->args['after']);
        }
    ...
    {
        $defaults = array('aria_label' => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'adeline'), 'home' => esc_html__('Home', 'adeline'), 'error_404' => esc_html__('404 Not Found', 'adeline'), 'archives' => esc_html__('Archives', 'adeline'),
            // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
            'search'                       => esc_html__('Search results for &#8220;%s&#8221;', 'adeline'),
            // Translators: %s is the page number.
            'paged'                        => esc_html__('Page %s', 'adeline'),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'               => esc_html__('Minute %s', 'adeline'),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'                 => esc_html__('Week %s', 'adeline'),
    ...
    protected function add_network_home_link()
    {
        if (is_multisite() && !is_main_site() && true === $this->args['network']) {
            $this->items[] = sprintf('<a href="%s" rel="home"><span class="dpr-icon-home"></span><span style="display: none;">%s</span></a>', esc_url(network_home_url()), $this->labels['home']);
        }
    ...
    protected function add_site_home_link()
    {
        ...
        $this->items[] = sprintf('<a href="%s"%s><span class="dpr-icon-home"></span><span style="display: none;">%s</span></a>', esc_url(home_url()), $rel, $label);
    }
    /**
    ...
    protected function add_minute_hour_archive_items()
    {
        ...
        if (true === $this->args['show_title']) {
            $this->items[] = sprintf($this->labels['archive_minute_hour'], get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'adeline')));
        }
    ...
    protected function add_minute_archive_items()
    {
        ...
        // Add the minute item.
        if (true === $this->args['show_title']) {
            $this->items[] = sprintf($this->labels['archive_minute'], get_the_time(esc_html_x('i', 'minute archives time format', 'adeline')));
        }
    ...
    protected function add_hour_archive_items()
    {
        ...
        // Add the hour item.
        if (true === $this->args['show_title']) {
            $this->items[] = sprintf($this->labels['archive_hour'], get_the_time(esc_html_x('g a', 'hour archives time format', 'adeline')));
        }
    ...
    protected function add_day_archive_items()
    {
        ...
        $year  = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'adeline')));
        $month = sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'adeline')));
        $day   = sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'adeline')));
    ...
    protected function add_week_archive_items()
    {
        ...
        $year = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'adeline')));
        $week = sprintf($this->labels['archive_week'], get_the_time(esc_html_x('W', 'weekly archives date format', 'adeline')));
        // Add the year item.
        $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y'))), $year);
        // Add the week item.
        if (is_paged()) {
    ...
    protected function map_rewrite_tags($post_id, $path)
    {
        ...
        if (is_array($matches)) {
            ...
            foreach ($matches as $match) {
                ...
                if ('%year%' == $tag) {
                    $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y', $post_id))), sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'adeline'))));
                }

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ('%monthnum%' == $tag) {
                    $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'adeline'))));
                }

                // If using the %day% tag, add a link to the daily archive.
                elseif ('%day%' == $tag) {
                    $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id))), sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'adeline'))));
                }
