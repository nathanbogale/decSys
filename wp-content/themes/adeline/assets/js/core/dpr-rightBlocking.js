...
function dpradelineRightBlocking() {

	"use strict"

	if ( $j( 'body' ).hasClass( 'blocked-right-click' ) ) {
		$j( window ).on( 'contextmenu', function( b ) {
			if ( 3 === b.which ) {
				showBlocker();
				return false;
			}
	...
	function showBlocker() {
		$j( '#rc-blocker' ).fadeIn( 'normal' , function() {
			$j( 'body' ).addClass( 'rc-blocker-displayed' );
		});
		$j( '#rc-blocker' ).on( 'click' , function() {
			$j( this ).fadeOut( 'normal' , function() {
				$j( 'body' ).removeClass( 'rc-blocker-displayed' );
			});
		});
	}
