<?php
if(!class_exists("UmayaCreativePortfolioAgencyWordPressThemeBase")) {
	class UmayaCreativePortfolioAgencyWordPressThemeBase {
    	public $key = "7768D726230C9277";
    	private $product_id = "38";
    	private $product_base = "umaya";
    	private $server_host = "https://webredox.net/licence/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_umaya_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=umaya_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
