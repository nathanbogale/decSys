...
namespace HolmesCore\CPT\Shortcodes\ImageGallery;
...
class ImageGallery implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Image Gallery', 'holmes-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Gallery Type', 'holmes-core' ),
							'value'       => array(
								esc_html__( 'Image Grid', 'holmes-core' ) => 'grid',
								esc_html__( 'Masonry', 'holmes-core' )    => 'masonry',
								esc_html__( 'Slider', 'holmes-core' )     => 'slider',
								esc_html__( 'Carousel', 'holmes-core' )   => 'carousel'
							),
							'save_always' => true,
							'admin_label' => true
						),
						array(
							'type'        => 'attach_images',
							'param_name'  => 'images',
							'heading'     => esc_html__( 'Images', 'holmes-core' ),
							'description' => esc_html__( 'Select images from media library', 'holmes-core' )
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'image_size',
							'heading'     => esc_html__( 'Image Size', 'holmes-core' ),
							'description' => esc_html__( 'Enter image size. Example: thumbnail, medium, large, full or other sizes defined by current theme. Alternatively enter image size in pixels: 200x100 (Width x Height). Leave empty to use "full" size', 'holmes-core' )
						),
						array(
							'type'        => 'dropdown',
						...
						array(
							...
							'heading'    => esc_html__( 'Image Behavior', 'holmes-core' ),
							'value'      => array(
								esc_html__( 'None', 'holmes-core' )             => '',
								esc_html__( 'Open Lightbox', 'holmes-core' )    => 'lightbox',
								esc_html__( 'Open Custom Link', 'holmes-core' ) => 'custom-link',
								esc_html__( 'Zoom', 'holmes-core' )             => 'zoom',
								esc_html__( 'Grayscale', 'holmes-core' )        => 'grayscale'
							)
						),
						array(
							'type'        => 'textarea',
							'param_name'  => 'custom_links',
							'heading'     => esc_html__( 'Custom Links', 'holmes-core' ),
							'description' => esc_html__( 'Delimit links by comma', 'holmes-core' ),
							'dependency'  => array( 'element' => 'image_behavior', 'value' => array( 'custom-link' ) )
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'custom_link_target',
							'heading'    => esc_html__( 'Custom Link Target', 'holmes-core' ),
						...
						//	'heading'     => esc_html__( 'Enable Slider Padding', 'holmes-core' ),
						//	'description' => esc_html__( 'Padding left and right on stage (can see neighbours).', 'holmes-core' ),
						//	'value'       => array_flip( holmes_mkdf_get_yes_no_select_array( false ) ),
						//	'save_always' => true,
	...
	public function render( $atts, $content = null ) {
		...
		$html = holmes_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'image-gallery', '', $params );
