		...
		<?php foreach ( $images as $image ) { ?>
			...
			if ( ! empty( $image_size_class ) ) {
				...
				if ( $image_size_class === 'large-width-height' ) {
					...
					$new_image_size = 'holmes_mkdf_square';
				} else if ( $image_size_class === 'large-width' ) {
					$new_image_size = 'holmes_mkdf_landscape';
				} else if ( $image_size_class === 'large-height' ) {
					$new_image_size = 'holmes_mkdf_portrait';
				} else {
					$new_image_size = 'full';
				}
			}
				...
				<div class="mkdf-ig-image-inner">
					...
					<?php } ?>
						<?php if ( is_array( $image_size ) && count( $image_size ) ) :
							echo holmes_mkdf_generate_thumbnail( $image['image_id'], null, $image_size[0], $image_size[1] );
						else:
