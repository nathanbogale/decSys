...
if($folio->have_posts()):
    ...
    <div class="row">
        <div class="col-lg-12">
            <?php 
                ...
                {
                    echo '<ul class="folioNav text-center">';
                        echo '<li class="active filter" data-filter="all">'.  esc_html__('All', 'yoox').'</li>';
                        foreach($categories as $cat)
                        {
                            echo '<li class="filter" data-filter="'.esc_attr($cat->slug).'">'.esc_html($cat->name).'</li>';
                        }
                    echo '</ul>';
                }
            ?>
    ...
    <?php endif; ?>
    <div class="row <?php if($is_filter == 'yes'){ echo 'folioFilter';} ?>">
    <?php
        $filterClass = '';
        if($is_filter == 'yes'){
            $filterClass = 'mix all ';
        ...
        while($folio->have_posts()):
            ...
            ?>
            <div class="col-lg-4 col-sm-6 col-md-4 <?php echo esc_attr($filterClass.' '.$classes); ?>">
                <div class="singlefolio">
                    <?php if(has_post_thumbnail()): ?>
                        <?php echo get_the_post_thumbnail(get_the_ID(), 'yoox-folio'); ?>
                    <?php else: ?>
